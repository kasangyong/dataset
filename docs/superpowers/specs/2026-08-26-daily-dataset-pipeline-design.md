...
| arXiv 신규 논문 | `arxiv_papers` | arXiv Atom API | 3 | 가능 | 일 |
...
3. **전멸 감지** — 드라이버가 **이번 실행에서 단 하나도 성공하지 못했을 때** exit code 1을
   반환한다. 일부만 실패하면 exit 0으로 두어 수집된 데이터는 커밋한다.
   이 층이 없으면 API가 전부 죽은 날에도 워크플로가 초록불로 끝나고 빈 날짜가 조용히 쌓인다.

   처음에는 **날짜별**로 "그 날짜의 모든 소스가 실패했는가"를 봤다. 모든 소스가 D-1 을
   채우던 시절에는 맞는 판정이었으나, `lag_days` 가 소스마다 갈라진 뒤로는 틀렸다.
   `arxiv_papers` 는 `lag_days=3` 인 유일한 소스라 그 날짜 칸에 혼자 있고, 그것 하나가
   실패하면 "그날 100% 실패"가 되어 나머지 9개가 정상인데도 잡이 빨간불이 됐다
   (2026-09-08 실제 발생). 데이터는 멀쩡한데 경보만 울리면 경보를 무시하게 된다.
   날짜별 전멸은 여전히 경고로 남기되 종료 코드는 실행 단위로 판정한다.
...
arXiv 검색 색인은 하루치가 한 번에 들어오지 않고 며칠에 걸쳐 채워진다. 실측:

| 수집 시점 | 건수 |
|---|---|
| 며칠 뒤 백필한 08-21~08-27 | 159~344 |
| 다음날 자동 수집한 09-01~09-04 | 41~77 |

`total == fetched` 를 확인했으므로 우리가 자른 것이 아니라, 그 시점에 arXiv가 실제로 그만큼만
갖고 있었다. 08-29 는 최초 0건이었다가 사흘 뒤 재시도에서 43건, 08-30 은 0 → 133 이 됐다.

**여기서 자동 복구의 한계가 드러난다.** 복구는 실패했거나 없는 파티션만 본다. 41건으로
*성공한* 파티션은 건강한 것으로 판단해 다시 보지 않으므로 영구히 부족한 채 남는다.

대응은 두 겹이다.

1. `lag_days=3` — 색인이 어느 정도 자리 잡은 뒤 수집한다. 값은 **잠정치다.**
   실제 안정 소요일을 재려다 arXiv 요청 제한(429)에 걸려 측정하지 못했다.
2. `recheck_days=3` — 최근 3일을 성공 여부와 무관하게 다시 읽는다.
   `merge_key="arxiv_id"` 를 함께 요구한다. 덮어쓰기라면 일시적으로 빈약한 응답이 온전한
   파티션을 지울 수 있지만, 병합은 더할 수만 있다. `test_registry` 가 이 짝을 강제한다.

**요청 제한 가드.** 오늘 몫 수집이 실패한 소스는 그날 다시 읽기를 건너뛴다. 실제로 진단 중
arXiv 에 429 를 맞았고, 그 상태에서 재확인이 요청을 4배로 늘리면 차단이 깊어질 뿐이다.
그래서 실행 순서를 바꿔 오늘 몫을 먼저 돌리고, 거기서 실패한 소스는 제외한다.
